...
class Msq {
    ...
    static double sarrival = START;     /* keeping track of the total time accumulated for arrivals and managing the exact time when the next arrivals will take place */
    ...
    public static void main(String[] args) {
        ...
        int s;                          /* server index                                                                                                       */
        long index = 0;                 /* used to count total processed jobs                                                                                 */
        double area = 0.0;              /* time integrated number in the node / keeps track of the sum of the number of jobs in the queue over time intervals */
        double service;
        ...
        MsqSum[] sum = new MsqSum[SERVERS + 1];         /* keep stats for every event */
        for (s = 0; s < SERVERS + 1; s++) {
            event[s] = new MsqEvent();
            sum[s] = new MsqSum();
        }

        MsqT t = new MsqT();            /* time sever tracking */
        t.current = START;

        event[0].t = m.getArrival(r);   /* set first arrive event                   */
        event[0].x = 1;                 /* event must be considered into simulation */
        ...
        while ((event[0].x != 0) || (number != 0)) {
            ...
            if (e == 0) {                               /* process an arrival*/
                ...
                number--;
                s = e;
                if (number >= SERVERS) {                /* there is some jobs in queue, place another job in this server */
                    service = m.getService(r);
                    event[s].t = t.current + service;
